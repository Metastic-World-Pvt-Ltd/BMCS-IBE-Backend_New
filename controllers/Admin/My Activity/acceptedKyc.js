...
module.exports.acceptedKyc = async function (req, res) {
    try {
        //user input
        ...
        try {
            ...
            const filteredItems = await Kyc.find({ acceptedBy: userEmail });
            const totalFilteredItems = filteredItems.length;
            // Use Mongoose to find paginated items
            const paginatedItems = await Kyc.find({ acceptedBy: userEmail })
                .skip((page - 1) * limit)
                .limit(limit);
